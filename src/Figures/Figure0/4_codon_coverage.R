...
ribocovtrs <- iso_tx_countdata$abundance%>%
	...
	left_join(ids_nrgname%>%distinct(transcript_id,gene_id))%>%
	group_by(gene_id)%>%
...
cdsseq <- cds2use%>%sort_grl_st%>%extractTranscriptSeqs(x=fafile)

cdsseq%>%translate%>%writeXStringSet('pipeline/codon_cov.prot.fa')
