...
# rustprofiledat%>%
	...
	filter(numreadlen>=26,numreadlen<=29)%>%
	arrange(position)%>%
	{
...
# codondata <- rustprofiledat%>%
    ...
    # filter(position == -offset-2)%>%
    filter(between(position, -offset-2,-offset))%>%
    group_by(assay,time,codon,rep)%>%
    summarise(dwell_time = sum(count))%>%
    arrange(assay!='ribo')
...
codondata%<>%
    ...
    arrange(assay!='ribo')%>%
    mutate(aacor_dwell_time=ifelse(n()==1,NA,dwell_time-mean(dwell_time)))
#summarise the replicates
repsumcodondata<-codondata%>%
	group_by(assay,time,codon,AA)%>%
	summarise_at(vars(abundance,availability,poly_abundance,poly_availability,dwell_time,aacor_dwell_time),mean)%>%
    arrange(assay!='ribo')
...
codondata%>%filter(assay=='ribo')%>%group_by(assay,time)%>%group_slice(1)%>%{quicktest(.$dwell_time,.$availability)}
repsumcodondata%>%filter(assay=='ribo')%>%group_by(assay,time)%>%group_slice(1)%>%{quicktest(.$dwell_time,.$availability)}

# repsumcodondata%>%filter(assay=='Polyribo')%>%group_by(assay,time)%>%group_slice(1)%>%{quicktest(.$dwell_time,.$poly_abundance)}
