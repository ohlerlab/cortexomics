...
fig2 <- function(){
  ...
  "weightedusage", "balance")]
	...
	# codonoccs<- timecodon_dt%>%select(time,codon,occupancy=codon_dwell_time)
	codonoccs<- codonprofiledat%>%left_join(offsets%>%transmute(readlen=as.character(length),offset))%>%filter(position== -offset-3)%>%
		separate(sample,c('time','assay','rep'))%>%group_by(time,codon)%>%summarise(occupancy = mean (count))
	# # pdf('plots/figures/figure2/trna_codons/codon_variance.pdf',w=12,h=12)
