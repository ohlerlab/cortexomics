...
normalizePath('plots/figures/figure2/trna_codons/offsetwindow_rlmerge_codprofs.pdf')
	...
	aa_corrected_occ_av_df%>%filter(time=='E13',fraction=='80S')%>%filter(availability< -1500)
	...
	tRNA_occ_df_mean = tRNA_occ_df_en%>%group_by(AA,codon)%>%summarise_at(vars('availability','dwell_time'),mean)
