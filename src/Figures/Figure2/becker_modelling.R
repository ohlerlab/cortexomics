...
notmsdevgenes = modeltestdf%>%filter(data=='riboseq')%>%group_by(gene)%>%filter(n()==5)%>%filter(BIC[model=='production']<BIC[model=='msdev'])%>%.$gene%>%unique
...
if(FALSE){	
	...
	# lowfiltergenes = modeltestdf%>% filter(data=='riboseq')%>% filter
	# (passtest[model=='production'],best
	# [model=='production'])%>% .$gene%>% unique
...
estimate = bmodelopts[gns_by_mod$msdev]%>%map_df(.id='gene',possibly(NULL,.f=.%>%.[['riboseq']]%>%.[['msdev']]%>%.$par%>%unlist))
msdevmat <- estimate%>%select(matches('msdev'))%>%select(-msdev1)%>%as.matrix
library(ComplexHeatmap)
devhm=  Heatmap(
            msdevmat[,4:1],cluster_columns=TRUE
          )
plotfile <- here(paste0('plots/','msdevhm','.pdf'))
pdf(plotfile)
devhm
dev.off()
message(normalizePath(plotfile))

genebmodels%>%enframe('gene_name','best_model')%>%write_tsv('tables/best_traj_model.tsv')

modeltestdf%>%
	ungroup%>%
	filter(data=='riboseq')%>%
	select(-data)%>%
	select(-matches('residuals'),-sumresid,-errorsum,-pval)%>%
	write_tsv('tables/traj_model_df.tsv')