...
################################################################################
if(!exists('psitecov')) base::source('src/Figures/Figure2_redo/3_metacodons.R')
...
# fpcovlist[[1]]->sampfpcov
# names(fpcovlist[[1]])->sampname
if(!file.exists(here('data/psitecov.rds'))){
	psitecov<-readRDS(here('data/psitecov.rds'))
}
psitecov_rlsum <- psitecov%>%lapply(function(x)purrr::reduce(x,`+`))

if(!file.exists(here('data/psiteprofilelist.rds'))){
	#we want the set of transcripts that always have at least 32 reads
	psitetrsums <- samp_rl_reads%>%
		lapply(.%>%
			lapply(.%>%
				subsetByOverlaps(innercds)%>%seqnames%>%table%>%{setNames(as.vector(.),names(.))}
			)%>%
			purrr::reduce(.,`+`)
		)
	tr_highcountdf <- psitetrsums%>%map_df(.id='sample',~(.>32)%>%enframe('tr_id','highcount'))%>%group_by(tr_id)%>%filter(all(highcount))
	allhighcountgenes = tr_highcountdf%>%filter(sample%in%mainsamps)%>%filter(all(highcount))%>%.$tr_id%>%unique
	#
	dtselgenelist <- list(all=ribocovtrs,allhighcount = allhighcountgenes)
	#
	sampname = names(psitecov)[[1]]
	seltrs = dtselgenelist[[1]]
	sampfpcov<-psitecov[[1]]
	rlfpcov<-sampfpcov[['29']]
	#
	psiteprofilelist <-psitecov[mainsamps[1]]%>%mcmapply(mc.cores=10,.,names(.),F=function(sampfpcov,sampname){
		# sampfpcov = psitetrsums[[sampname]][seltrs]
		subfpprofilelist <-lapply(dtselgenelist[1],function(seltrs){
			trsums = sampfpcov%>%map(.%>%sum)%>%map(~.[seltrs])%>%Reduce(`+`,.)
			# trsums = psitetrsums[[sampname]][seltrs]
			sampfpcov['29']%>%lapply(function(rlfpcov){
				rlfpcov = rlfpcov[seltrs]/(trsums)
				# stopifnot(!any)
	...
	#could speed this up I think
	# codbpsempty = psitecov[[1]][[1]][allcodlistnz]
	# psiteprofilelist <-psitecov[mainsamps]%>%mcmapply(mc.cores=10,.,names(.),F=function(sampfpcov,sampname){
	# 	subfpprofilelist <-lapply(dtselgenelist[1],function(seltrs){
	# 		trsums = psitetrsums[[sampname]][seltrs]
	# 		sampfpcov%>%lapply(function(rlfpcov){
	# 			rlfpcov = rlfpcov[seltrs]/(trsums)
	# 			sampreads[[1]][[1]]%>%subsetByOverlaps(allcodlistnz)%>%coverage%>%.[allcodlistnz]
	# 			sampreads[[1]][[1]]%>%resize(1)%>%subsetByOverlaps(allcodlistnz)
	# 			# psitecov = sampreads%>% psitefunc
	# 			allcodlistnz = allcodlist%>%subset(seqnames%in%names(trsums)[trsums!=0])
	# 			cods = names(allcodlistnz)%>%str_split('\\.')%>%map_chr(1)
	# 			out = rlfpcov[allcodlistnz]%>%split(cods)%>%lapply(as.matrix)%>%map(colMeans)
	...
	# 	})
	# })
	saveRDS(psiteprofilelist,here('data/psiteprofilelist.rds'))
}else{
	psiteprofilelist<-readRDS(here('data/psiteprofilelist.rds'))
}


codonprofiledat = psiteprofilelist%>%map_depth(3,.%>%enframe('position','count'))%>%map_df(.id='sample',.%>%map_df(.id='readlen',.%>%bind_rows(.id='codon')))
codonprofiledat%<>%separate(sample,c('sample','subgroup'),sep='\\.')
...
# codonprofiledat <- psiteprofilelist%>%map_df(.id='subgroup',.%>%map_depth(3,.%>%enframe('position','count'))%>%map_df(.id='sample',.%>%map_df(.id='readlen',.%>%bind_rows(.id='codon'))))
...
# 	fprustprofilelist <-fpcovlist[mainsamps[1]]%>%mclapply(.,names(.),FUN=function(sampfpcov,sampname){
...
# 		)
# 	)
#this was for adjusting when we had fp sites instead of psites
...
{
if(!exists('cdsseqs')){
	cdsseqs = cdsgrl[ribocovtrs]%>%sort_grl_st%>%extractTranscriptSeqs(x=fafileob)
}
