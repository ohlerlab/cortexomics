...
allreadnumstats <- lapply(pdirs,function(pdir){
	...
	if(pdir=='Ovarian_cancer_ribo'){
		cutadaptfiles <- Sys.glob(paste0(pdir,'/ext_data/cutadapt*/*'))
 		readttotalreads <-cutadaptfiles%>%setNames(.,str_replace(basename(.),'cutadapt.stats',''))%>%map_dbl(.%>%readLines%>%str_subset('input')%>%head(1)%>%str_extract('[0-9,]+$')%>%str_replace_all(',','')%>%as.numeric)
 		totalreads <- rep(NA,length(samples))
 		totalreads[match(names(readttotalreads),toupper(str_extract(samples,'rpi\\d+')))]<-readttotalreads
 		totalreads%<>%setNames(samples)

	}else{
		cutadaptfiles <- Sys.glob(paste0(pdir,'/pipeline/cutadapt_reads/*/*.fastq.gz.cutadaptstats.txt'))
		totalreads<-cutadaptfiles%>%setNames(.,str_replace(basename(.),'cutadapt.stats',''))%>%map_dbl(.%>%readLines%>%head(8)%>%tail(1)%>%str_extract('[0-9,]+$')%>%str_replace_all(',','')%>%as.numeric)
...
readnumstatdf<-allreadnumstats%>%setNames(pdirs)%>%bind_rows(.id='project')%>%group_by(sample)%>%filter(sample%>%
...
readnumstatdf%>%
	...
	identity%>%
	select(project,sample,matches('Million'))%T>%
	write_tsv('project_readuse_stats.tsv')

readnumstatdf$project%>%unique
readnumstatdf%>%filter(project=='Ovarian_cancer_ribo')
