...
setwd('~/bih_cluster/fast/groups/ag_ohler/work/dharnet_m/cortexomics/')
...
selms_tall_summ <- selms_tall %>% 
  ...
  group_by(pcat,Protein_IDs,time,fraction)%>%summarize(signal=median(na.rm=T,signal))%>%
...
plotfile<-'./plots/ribosto_trajectories.pdf'
pdf(plotfile,w=13,h=4)
print(traplot+a)
dev.off();normalizePath(plotfile)

#test with nest
tps <- unique(selms_tall$time)
lapply(tps[-1],function(tp){
selms_tall%>%group_by(pcat,fraction)%>%filter(time %in% c('E13',tp))%>%
    nest%>%
  ...
  dplyr::select(-data)%>%
  mutate(time=tp)
})%>%bind_rows%>%write_tsv('tables/protein_group_traj_plot_sig.txt')
'tables/protein_group_traj_plot_sig.txt'%>%normalizePath
