...
transregprotids <- getBM(attributes = c("uniprot_gn"), 
                                  filters=('go'),values=transreggoterm,
                                  mart = mart)
translationgoterm <- "GO:0006417"
translationprotids <- getBM(attributes = c("uniprot_gn"), 
...
make_traplot <- function(data,sigtests){
    ...
    scale_shape_manual(name='Sig Diff from E13',values=setNames(c('circle','circle','triangle'),c(NA_character_,'FALSE','TRUE')))+
    theme_minimal()+
    theme(panel.grid = element_blank(),axis.title.y = element_text(size=20),strip.text.x = element_text(size=20))
  }
#
#
sigs<-tpchangesig(selms_tall)
#
traplot <-  selms_tall_summ%>%make_traplot(data=.,sigs)


rnatraplot <- rnaseqdata %>%make_traplot(.,tpchangesig(rnaseqdata_tall%>%mutate(fraction='RNAseq'))) 
