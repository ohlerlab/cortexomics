...
setMethod('seqinfo','DNAStringSet',function(seq){
	seq%>%{Seqinfo(names(.),nchar(.))}
})
...
get_seqforrest_traindata <- function(reads,seq,nbp=2){
	...
	stopifnot(all(seqnames(reads)%in%names(seq)))
...
get_seqoffset_model <-  function(data4readshift){
	...
	shiftforrestfit <- ranger::ranger(formula= factor(dist) ~ . ,
		data=data4readshift,
...
get_predictedshifts<-function(shiftforrestfit,data4readshift){predict(shiftforrestfit,data=data4readshift)$prediction%>%as.character%>%as.numeric}
...
c(topcdsexons,topcds,topcdsmap) %<-%  mymemoise(get_top_trs)(bam,exons,cds,startcods)
...
c(bestscores,offset_cds_scores) %<-% mymemoise(get_offsets)(cdsread_trmap,compartments)
...
#so the stop site selection is now identical



shiftforrestfit <- get_seqoffset_model(traindata4readshift)
...
# cdsread_trmap_inbound$seqshift%>%table
