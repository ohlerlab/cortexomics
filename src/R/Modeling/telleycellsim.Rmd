...
shared_names <- ft_agg_df[min_ss_sig>8,]$gene_name%>%intersect(mybulkdf$gene_name)
...
deconpropdf%>%group_by(dset,ft_diff)%>%summarise(prop=sum(prop))%>%spread(ft_diff,prop)
