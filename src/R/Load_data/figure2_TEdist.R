...
conflict_prefer('type','Biostrings')
...
techanges%<>%semi_join(gnamehighcountdf)

techanges%>%group_by(time)%>%group_slice(1)%>%filter(adj_p_value<0.05)

pdf('plots/figures/figure2/TEchange_horizontaldist.pdf',w=12,h=6)
...
fcdirnums<-techanges%>%filter(feature_id%in%highcountgenes)%>%group_by(time,sign(log2fc))%>%
filter(adj_p_value<0.05)%>%
filter(abs(log2fc)>1)%>%
tally
fcdirnums%>%tally
...
tr_gid_df<-cds%>%mcols%>%data.frame%>%distinct(transcript_id,gene_id)
...
pdf(h=5,w=5,'plots/figures/figure2/cumdist_TEchange_fplen.pdf')
allTEchangedf%>%
	left_join(transcript_fp_utrlen%>%left_join(tr_gid_df)%>%group_by(gene_id)%>%slice(which.max(fputr_length)))%>%
...
pdf(h=5,w=5,'plots/figures/figure2/cumdist_TEchange_tplen.pdf')
allTEchangedf%>%
	left_join(transcript_tp_utrlen%>%left_join(tr_gid_df)%>%group_by(gene_id)%>%slice(which.max(tputr_length)))%>%
...
conflict_prefer("plotMA", "BiocGenerics")
pdf(h=5,w=5,'plots/figures/figure2/cumdist_TEchange_stopchange.pdf')
plot1=allTEchangedf%>%
...
pdf(h=5,w=5,'plots/figures/figure2/stop_madist.pdf')
...
pdf('plots/figures/figure1/abs_TE_vs_start_occ.pdf')
...
# pdf(h=5,w=5,'plots/figures/figure2/stop_madist_hist.pdf')
...
conflict_prefer("plotMA", "BiocGenerics")
pdf(h=5,w=5,'plots/figures/figure2/cumdist_TEchange_startchange.pdf')
plot1=allTEchangedf%>%
...
library(ggExtra)
# 
pdf(h=5,w=5,'plots/figures/figure2/start_madist.pdf')
