...
if(!isknitr) rmarkdown::render(knitr::spin('cortexomics/src/R/load_telley_etal.R',knit=F),output_dir=here('Reports'),knit_root_dir=here())
...
xtaildata%<>%rename('feature_id'='gene_id')
...
test_geneset_ctl_expr<-function(gnames2test,testname,tegenes=tegenes,matchsettbl=matchsettbl,symid=symid){
...
purely(test_geneset_ctl_expr)(c(t_dynamicstbl$gene_name,t_wavetbl$gene_name),testname='All_Wave_Genes',tegenes,matchsettbl=matchsettbl,symid=symid)
0
...
exprdata%>%filter(gene_name%in%t_timecoretbl$gene_name,assay=='ribo')%>%.$signal%>%hist(50)
