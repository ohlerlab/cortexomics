...
#load up the rnaseq segment counts
#figure out which ones we need for the mass spec
#get the RNAseq counts for those
#

args <- c(
	segment_counts_list = paste0(collapse=',',Sys.glob(here('pipeline/seqshift_reads/data/*/segment_counts_df.tsv'))),
...
#get rnaseq counts for the riboseq protein counts that are 
memsumov <- mymemoise(function(gr,bam) GenomicAlignments::summarizeOverlaps(gr,bam))
rnaseqfiles <- Sys.glob(here('pipeline/star/data/*total*/*.bam'))%>%str_subset(neg=TRUE,'transcript')
rnaseqcounts <-  GenomicAlignments::summarizeOverlaps(cds_nz%>%split(.,.$protein_id),rnaseqfiles)
