...
cdsids2use<-countexprdata%>%fData%>%as.data.frame%>%filter(gene_id%in%highcountgenes,is_gid_highest)%>%.$protein_id%>%unique
cds2use <- cds%>%subset(strand=='+')%>%subset(protein_id%in%cdsids2use)%>%split(.,.$protein_id)
...
{
startstopprofiles<-mclapply(mc.cores=20,fpsitelist,(function(reads){
	# reads%<>%resize(.,.$length)
	#
